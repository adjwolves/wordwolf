...
describe("RoomController (e2e)", () => {
  ...
  describe("/room (POST)", () => {
    it("should return 201", () => {
      ...
      return testRequest(app.getHttpServer()).post("/room").send(createRoomDto).expect(201);
    });
    it("should return 400", () => {
      return testRequest(app.getHttpServer()).post("/room").send({}).expect(400);
    });
  });
});
...
describe("EventsGateway (e2e)", () => {
  ...
  describe("join-room", () => {
    it("should return sessionId", async () => {
      const createRoomDto = new CreateRoomDto();
      createRoomDto.category = "fruit";
      const response = await axios.post("http://localhost:3001/room", JSON.stringify(createRoomDto));
